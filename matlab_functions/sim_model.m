...
out.pcor = mean(bsxfun(@eq,r,dat.s),[1,2]); % response accuracy
...
out.cfg = cfg;
    ...
    function [r] = simr_inflaps(h,siginf,sigsel,plaps)
        ...
        for itrl = 1:ntrl
            ...
            x(itrl,~i0) = x(itrl,~i0)+s(~i0)*logit(1-pfal);
    ...
    function [r] = simr_infwght(h,siginf,sigsel,delta)
        ...
        for itrl = 1:ntrl
            ...
            s = dat.s(itrl)+sigsen*randn(1,nsmp);
            s = s*2/sigsen^2;
            iphi = s > +delta; % above threshold => xhi
            inhi = s < -delta;
            iplo = s < +delta & s > 0; % below threshold
            inlo = s > -delta & s < 0;
            % add log-evidence ABOVE THRESHOLD

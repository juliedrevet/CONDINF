function [out] = fit_model_revrep_leak(cfg)
%  FIT_MODEL_REVREP  Fit leaky inference model to reversal and repetition curves
%
...
%
% The non-linear update rule function PSY (Glaze & Gold, 2015) is approximated 
% by a linear function: PSY(prior_belief,h) ~ (1-2*h)*prior_belief => leaky
% accumulator

% check presence of required parameters
if ~all(isfield(cfg,{'t','s'}))
...
if all(isfield(cfg,{'crev','crep'}))
    if numel(cfg.crev) ~= 8 || numel(cfg.crep) ~= 8
        error('Wrong size for precomputed reversal metrics!');
    end
    % use precomputed reversal/repetition curves
    use_cdat = true;
    dat.crev = cfg.crev(:); % reversal curve
    dat.crep = cfg.crep(:); % repetition curve
else
...
nrev = nnz(dat.s(2:end) ~= dat.s(1:end-1) & dat.t(2:end) > 1);

% get model type
modtype = lower(cfg.modtype);
if ~ismember(modtype,{'flatinf','nonstab','senbias','inflaps','infdisc','selbias','selepsi','infwght'})
...
sigsen = 1/norminv(1-pfal); % sensory noise
...
if use_cdat
    ...
    csub = getc(dat.t,dat.s,dat.r);
    crev_sub = csub.rev_avg; % reversal curve
    crep_sub = csub.rep_avg; % repetition curve
end
...
switch modtype
    ...
    case 'infdisc' % confitional inference
        % 4/ discard criterion
        pnam{1,4} = 'delta';
        pmin(1,4) = 0;
        pmax(1,4) = 5;
    ...
    case 'infwght' % weighted inference (added after review)
        % 4/ weight criterion
        pnam{1,4} = 'delta';
        pmin(1,4) = 0;
        pmax(1,4) = 5;
...
out.crev_sub = crev_sub; % reversal curve
out.crep_sub = crep_sub; % repetition curve

% store best-fitting reversal metrics
chat = simc(phat{:});
% reversal curve
out.crev_avg = chat.rev_avg;
out.crev_std = chat.rev_std;
% repetition curve
out.crep_avg = chat.rep_avg;
out.crep_std = chat.rep_std;
...
out.cfg = cfg;
    ...
    function [ll,ll_sd] = getll(varargin)
        ...
        for ires = 1:nres
            ...
            crev_avg = mean(c.rev_all(:,jres),2);
            crep_avg = mean(c.rep_all(:,jres),2);
            if std_sim
                % use simulations uncertainty
                crev_std = std(c.rev_all(:,jres),[],2);
                crep_std = std(c.rep_all(:,jres),[],2);
            else
        ...
        ll = ...
            sum(normllh(crev_sub,c.rev_avg,max(c.rev_std,1e-3)))+ ...
            sum(normllh(crep_sub,c.rep_avg,max(c.rep_std,1e-3)));
    end
    ...
    function [c] = simc_inflaps(h,siginf,sigsel,plaps)
        ...
        for itrl = 1:ntrl
            ...
            x(itrl,~i0) = x(itrl,~i0)+s(~i0)*logit(0.8); %logit(1-pfal);
    ...
    function [c] = simc_selbias(h,siginf,sigsel,beta)
        ...
        for itrl = 1:ntrl
            ...
            s = sign(dat.s(itrl)+sigsen*randn(1,nsmp));
            % add log-evidence
            x(itrl,:) = x(itrl,:)+s*logit(0.8); %logit(1-pfal);
            % add inference noise
            x(itrl,:) = x(itrl,:)+siginf*randn(1,nsmp);
            % sample response with selection noise
            if dat.t(itrl) == 1
...
% y = x+log((1-h)./h+exp(-x))-log((1-h)./h+exp(+x)); % PSY (Glaze & Gold,
% 2015)
y = (1-2*h)*x; % linear approximation of PSY
end

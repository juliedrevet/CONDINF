...
if isfield(cfg,'Yavg')
    nmodel_dat =  size(cfg.Yavg,2);
    for imod = 1:nmodel_dat
        plot(xvec,cfg.Yavg(:,imod),'ko','MarkerSize',10,'MarkerFaceColor',rgb_dat(imod,:));
    end
end

%%
set(gca,'Layer','top','Box','off','TickDir','out','PlotBoxAspectRatio',[pbar,1,1]);
set(gca,'XTick',xvec,'XTickLabel',{'-4','-3','-2','-1','1','2','3','4'},'YTick',0:0.1:max(ylim));
xlabel('trial position from reversal');
ylabel('fraction switch');
helper_ILL(gcf,pbar,HGT,mkr_siz);   
